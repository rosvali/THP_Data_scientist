module Journalist
	...
	def self.length
		...
		puts "> Il y a #{TAB.length} handle dans l'array"
		puts "----------"
	end
	
	def self.min
		puts "Quelle est le handle le plus court de cette liste ?"
		puts "> Le handle le plus court est #{TAB.min {|a, b| a.length <=> b.length}}"
		puts "----------"
	...
	def self.capitalize
		...
		puts "> Il y a #{TAB.count {|x| x[1] == x[1].upcase}} handles qui commencent par une majuscule"
		puts "----------"
	end
